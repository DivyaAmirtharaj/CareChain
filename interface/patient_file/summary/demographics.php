...
//   dlgopen('advancedirectives.php', '_blank', 400, 500);
   dlgopen('blockchain.php', '_blank', 400, 500);
  }
...
<div>
    ...
    if (isset($pid) && !$GLOBALS['disable_calendar'] && $GLOBALS['appt_recurrences_widget'] &&
        acl_check('patients', 'appt')) {
         $widgetTitle = xl("Blockchain Info");
         ...
         $recurrences = fetchRecurrences($pid);
          ...
          $query = "SELECT blockchain_status, substring(blockchain_publickey,1,20) blockchain_publickey, blockchain_cloud, blockchain_cloud_username FROM patient_access_onsite WHERE pid = ? AND blockchain_publickey is not null";
         $res = sqlStatement($query, array ($pid));
         $row = sqlFetchArray($res);
 
         //Blockchain Info Fectch
         
        if ($row['blockchain_status'] == 0) { 
            echo "<div>";
            echo "<span>" . "&nbsp;&nbsp;" . xlt('Non-Blockchain Patient') . "</span>";
            echo "</div></div>";
        } else {
                echo "<div>";
                echo "<span>" . xlt('Public Key') . ': ' . xlt($row['blockchain_publickey']) . "...</span>";
                echo "<br>";
                echo "<span>" . xlt('Cloud Storage') . ': ' . xlt($row['blockchain_cloud']) . "</span>";
                echo "<br>";
                echo "<span>" . xlt('Cloud Username') . ': ' . xlt($row['blockchain_cloud_username']) . "</span>";
                echo "</div>";
            echo "</div>";
        }
    }
