...
function generatePageElement($start, $pagesize, $billing, $issue, $text)
{
    ...
    $url="get_visits.php?"."pagestart=".attr($start)."&"."pagesize=".attr($pagesize);
    $url.="&billing=".$billing;
    $url.="&issue=".$issue;
...
function changePageSize()
{
    ...
    pagesize=$(this).val();
    top.restoreSession();
    window.location.href="get_visits.php?billing="+billing+"&issue="+issue+"&pagestart="+pagestart+"&pagesize="+pagesize;
}
...
if ($issue) {
    ...
    echo $attendant_type == 'pid' ? htmlspecialchars(xl('Past Visits and Documents'), ENT_NOQUOTES) : htmlspecialchars(xl('Past Therapy Group Encounters'), ENT_NOQUOTES);
}
?>
</font>
&nbsp;&nbsp;
...
?>
<?php if ($billing_view) { ?>
<a href='get_visits.php?billing=0&issue=<?php echo $issue.$getStringForPage; ?>' onclick='top.restoreSession()' style='font-size:8pt'>(<?php echo htmlspecialchars(xl('To Clinical View'), ENT_NOQUOTES); ?>)</a>
<?php } else { ?>
<a href='get_visits.php?billing=1&issue=<?php echo $issue.$getStringForPage; ?>' onclick='top.restoreSession()' style='font-size:8pt'>(<?php echo htmlspecialchars(xl('To Billing View'), ENT_NOQUOTES); ?>)</a>
<?php } ?>
