 ...
 */
    ...
    $sql = "SELECT id, title, date_format(begdate,'%m/%d/%Y') as begdate,  date_format(enddate,'%m/%d/%Y') as enddate, referredby, id FROM lists WHERE pid = ? AND type = 'medical_problem' ORDER BY id desc";
...
if (sqlNumRows($res)>0) {
    ...
    while ($row = sqlFetchArray($res)) {
          ...
          echo "<td style='padding:.75pt .75pt .75pt .75pt'>".text($row['date'])."</td>";
