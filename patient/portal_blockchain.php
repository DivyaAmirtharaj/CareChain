...
//CO-PAY
 ...
 if(ok==-1)
  {
     //return true;
     if(confirm("<?php echo addslashes(xl('Blockchain Validated: Save?')) ?>"))
      {
       return true;
      }
     else
...
$("#payfrm").on('submit', function(e){
    ...
    var flag = 0
    var liburl ='<?php echo $GLOBALS["webroot"] ?>/patient/lib/paylib.php';
    $.ajax({
        type: "POST",
        url: liburl,
        data: $("#payfrm").serialize()+extra,
...
<body class="body_top" onunload='imclosing()' onLoad="cursor_pointer();"
    ...
    <form id="payfrm" method='post'
        action='<?php echo $GLOBALS["webroot"] ?>/patient/portal_payment.php'>
        ...
        <table>
            ...
            <tr>
                <td colspan='3' align='center' class='text'><b><?php echo xlt('Update Blockchain for '); ?>&nbsp;:&nbsp;&nbsp;<?php
...
<?php
if (! isset($_SESSION['authUserID'])) {
    echo '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#openPayModal">' . xlt("Save") . '</button>';
} else {
